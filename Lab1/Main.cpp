...
// Global Variables:
MyImage			inImage, modImage;						// image objects
                     ...
                     int       nCmdShow)
{
	...
	int myc = 0;
	while (lpCmdLine[myc] != ' ' && lpCmdLine[myc] != 0) {
		myc++;
	}
	myc++;
	
	strcpy(pram, &lpCmdLine[myc]);
	...
	printf("The first parameter was: %s", pram);
	...
	modImage.setWidth(w);
	modImage.setHeight(h);
	modImage.setImagePath(lpCmdLine);
	modImage.ReadImage();
	//modImage.Modify(pram);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   hWnd = CreateWindowEx(0,szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
      CW_USEDEFAULT, 0, 4000, 4000, NULL, NULL, hInstance, NULL);
		...
		case WM_PAINT:
			{
				...
				bmi2.bmiHeader.biWidth = modImage.getWidth();
				bmi2.bmiHeader.biHeight = -modImage.getHeight();  // Use negative height.  DIB is top-down.
				bmi2.bmiHeader.biPlanes = 1;
				bmi2.bmiHeader.biBitCount = 24;
				bmi2.bmiHeader.biCompression = BI_RGB;
				bmi2.bmiHeader.biSizeImage = modImage.getWidth() * modImage.getHeight();



				SetDIBitsToDevice(hdc,
					inImage.getWidth() + 20, 100, modImage.getWidth(), modImage.getHeight(),
					0, 0, 0, modImage.getHeight(),
					modImage.getImageData(), &bmi2, DIB_RGB_COLORS);
				SetDIBitsToDevice(hdc,
