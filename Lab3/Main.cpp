...
// Global Variables:
MyImage			inImage, modImage, sImage[10];;						// image objects
                     ...
                     int       nCmdShow)
{
	...
	printf("The second parameter was: %s", pram);
		...
		case WM_PAINT:
			{
				...
				char text[1000];
				strcpy(text, "The image is shown as follows. \n");
				...
				if (strcmp(pram, "-1") != 0) {
					...
					while (count <= frame) {
						...
						next_frame += std::chrono::milliseconds(1000 / 1); // 5Hz
						//inImage = sImage[count];
						inImage.Modify(pram, count);
						count++;
						BITMAPINFO bmi;
						CBitmap bitmap;
						memset(&bmi, 0, sizeof(bmi));
