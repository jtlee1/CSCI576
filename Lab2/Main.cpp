...
char* Y, * U, * V, * A;
int frame = 10;
MyImage			inImage,bImage, sImage[480];						// image objects
HINSTANCE		hInst, hInst2;							// current instance
TCHAR szTitle[MAX_LOADSTRING];					// The title bar text
TCHAR szWindowClass[MAX_LOADSTRING];			// The title bar text
char pram[_MAX_PATH];
char pram2[_MAX_PATH];
                     ...
                     int       nCmdShow)
{
	...
	inImage.setWidth(w);
	inImage.setHeight(h);
	inImage.setImagePath(Y);
	inImage.ReadImage();
	
	
	bImage.setWidth(w);
	bImage.setHeight(h);
	bImage.setImagePath(U);
	bImage.ReadImage();
	inImage.setBk(&bImage);
	int count = 0;
	while (count < frame) {
		inImage.Modify(Y, U, count, V);
		sImage[count] = inImage;
		count++;
	}
		...
		case WM_PAINT:
			{
				...
				while(count<frame){
					...
					next_frame += std::chrono::milliseconds(1000 / 24); // 5Hz
